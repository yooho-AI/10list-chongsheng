...
function MobileHeader({
  ...
  return (
    <header
      ...
      style={{
        flexDirection: 'column', gap: 4, padding: '8px 12px 6px',
    ...
    >
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', width: '100%' }}>
        <div className="mobile-header-left">
          <span className="mobile-header-stage" style={{ color: '#1e293b', fontWeight: 600 }}>
        ...
        <div className="mobile-header-right">
          <button className="mobile-header-npc" onClick={onCharClick} style={{ color: '#1e293b' }}>
            {char ? (
              <span style={{ color: char.themeColor }}>{char.name}</span>
            ) : (
              <span style={{ color: '#64748b' }}>选择角色</span>
            )}
          ...
          return (
            <button
              ...
              style={{
                ...
                padding: '3px 10px',
                borderRadius: 99,
                border: active ? '1px solid #ff6b9d' : '1px solid #e2e8f0',
                background: active ? 'rgba(255,107,157,0.1)' : 'rgba(255,255,255,0.8)',
                color: active ? '#ff6b9d' : '#64748b',
                fontSize: 11,
...
function MobileLetterCard() {
  return (
    <div
      className="mobile-letter-card"
      style={{ background: 'rgba(255,255,255,0.95)', color: '#1e293b', border: '1px solid #e2e8f0' }}
    >
      <div className="mobile-letter-icon">👶</div>
      <div className="mobile-letter-genre" style={{ color: '#64748b' }}>{STORY_INFO.genre}</div>
      <h2 className="mobile-letter-title" style={{ color: '#1e293b' }}>{STORY_INFO.title}</h2>
  ...
  return (
    <div ref={scrollRef} className="mobile-dialogue cs-scrollbar" style={{ position: 'relative' }}>
      ...
      {messages.map((msg) => {
        if (msg.role === 'user') {
          return (
            <div key={msg.id} className="mobile-msg-user">
              <div
                className="mobile-bubble-user"
                style={{ background: 'linear-gradient(135deg, #ff6b9d 0%, #e91e8c 100%)', color: '#fff' }}
              >
                {msg.content}
              </div>
            </div>
        ...
        if (msg.role === 'system') {
          return (
            <div key={msg.id} className="mobile-msg-system" style={{ color: '#64748b' }}>
              {msg.content}
            </div>
          )
        }
      ...
      {isTyping && !streamingContent && (
        <div className="mobile-msg-ai">
          <div
            className="mobile-bubble-ai mobile-typing"
      ...
      >
        {canHighlight && (
          <button
            ...
            style={{ borderColor: '#ff6b9d', color: '#ff6b9d', background: 'rgba(255,107,157,0.06)' }}
          >
            ✨ 高光
          </button>
        )}
      ...
      {open && (
        <>
          ...
          >
            ...
            <div className="mobile-sheet-title" style={{ color: '#1e293b' }}>选择角色</div>
              ...
              {Object.values(available).map((char) => {
                ...
                return (
                  <button
                    ...
                    style={{
                      borderColor: isSelected ? char.themeColor : '#e2e8f0',
                      background: isSelected ? 'rgba(255,107,157,0.06)' : '#fff',
                    }}
    ...
    <AnimatePresence>
      {open && (
        <>
          ...
          >
            <div className="mobile-sheet-handle" style={{ background: '#cbd5e1' }} />
            <div className="mobile-sheet-title" style={{ color: '#1e293b' }}>🎒 背包</div>
            <div className="cs-scrollbar" style={{ maxHeight: '50vh', overflowY: 'auto' }}>
              {hasItems ? (
                  ...
                  {Object.entries(inventory).map(([itemId, count]) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        {count > 1 && (
                          <span style={{ fontSize: 12, color: '#94a3b8' }}>x{count}</span>
                        )}
                      </button>
                    )
                  })}
                ...
                <div className="cs-placeholder" style={{ height: 120 }}>
                  ...
                  <span className="cs-placeholder-text" style={{ color: '#94a3b8' }}>背包空空如也</span>
                </div>
              )}
            </div>
          </motion.div>
  ...
  return (
    ...
    >
      ...
      >
        ...
        <h2 style={{ fontSize: 20, fontWeight: 600, color: '#1e293b', margin: '0 0 12px' }}>
          {ending.name}
        </h2>
        <p style={{ fontSize: 13, lineHeight: 1.8, color: '#64748b', marginBottom: 20 }}>
          {ending.description}
        </p>
        <button
          onClick={() => resetGame()}
    ...
    <AnimatePresence>
      {open && (
        <>
          ...
          >
            <div className="mobile-menu-title" style={{ color: '#1e293b' }}>游戏菜单</div>
            <button
